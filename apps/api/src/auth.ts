...
export const publicUser = (
  user: Pick<User, "id" | "email" | "displayName" | "role" | "status" | "emailVerifiedAt">,
) => ({
  id: user.id,
  email: user.email,
    ...
    if (current.revokedAt || current.replacedById) {
      ...
      await tx.securityEvent.create({
        data: {
          ...
          severity: "HIGH",
          metadata: { family: current.tokenFamily },
        },
      });
...
export async function verifyPasswordAndTrack(prisma: PrismaClient, email: string, password: string) {
  ...
  if (!valid) {
    ...
    const lockedUntil = failures >= 5 ? new Date(Date.now() + Math.min(30, 2 ** (failures - 5)) * 60_000) : null;
    await prisma.user.update({ where: { id: user.id }, data: { failedLoginCount: failures, lockedUntil } });
    return null;
  }
