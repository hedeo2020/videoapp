...
test("availability windows include currently available catalog only", () => {
  ...
  assert.equal(
    isCurrentlyAvailable({ availabilityStart: new Date("2026-07-01"), availabilityEnd: new Date("2026-08-01") }, now),
    true,
  );
...
test("movie cards require a ready playback asset before surfacing", () => {
  ...
  assert.equal(
    movieHasReadyAsset({ assets: [{ id: "asset-1", state: "PROCESSING", durationSeconds: 60 }] } as never),
    false,
  );
});
